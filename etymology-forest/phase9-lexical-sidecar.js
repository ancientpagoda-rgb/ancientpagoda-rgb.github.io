...
let manifestPromise=null,manifest=null,available=null,lastFail=0;
...
async function getManifest(){if(available===false&&Date.now()-lastFail<30000)return null;if(manifest)return manifest;if(manifestPromise)return manifestPromise;manifestPromise=(async()=>{try{const r=await REAL_FETCH(BASE+'manifest.json?t='+Date.now(),{cache:'no-store'});if(!r.ok)throw new Error(String(r.status));manifest=await r.json();available=true;lastFail=0;return manifest}catch(_){available=false;lastFail=Date.now();return null}finally{manifestPromise=null}})();return manifestPromise}
...
async function loadShard(id){if(CACHE.has(id))return CACHE.get(id);const p=(async()=>{const v=encodeURIComponent(manifest?.generated_at||manifest?.version||'1'),r=await REAL_FETCH(BASE+id+'.jsonl.gz?v='+v,{cache:'force-cache'});if(!r.ok)return new Map();const text=await ungzip(r),m=new Map();for(const line of text.split(/\r?\n/)){if(!line)continue;try{const a=JSON.parse(line);if(a&&a.length>=6)m.set(key(a[0],a[1]),{meaning:a[2]||'',ipa:a[3]||'',roman:a[4]||'',pos:a[5]||''})}catch(_){}}return m})();CACHE.set(id,p);return p}
...
window.__lexicalSidecar={base:BASE,get available(){return available},get manifest(){return manifest},lookup,refresh(){available=null;lastFail=0;manifest=null;manifestPromise=null;CACHE.clear();return getManifest()}};
